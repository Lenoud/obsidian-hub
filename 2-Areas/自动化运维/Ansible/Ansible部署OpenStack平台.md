...
使用 Ansible 部署 OpenStack 云平台。

本文详细介绍如何使用 Ansible Playbook 一键部署单控制单计算的 OpenStack 云平台，涵盖环境准备、目录结构设计、角色剧本编写和最终执行的完整流程。
...
| 关键参数 | 值 | 说明 |
| --- | --- | --- |
| controller_ip | 172.128.11.21 | Controller 节点 IP |
| compute_ip | 172.128.11.20 | Compute 节点 IP |
| PASSWD | Abc@1234 | 统一密码 |
| cinder_disk | vdb1 | Cinder 块存储分区 |
| swift_disk | vdb2 | Swift 对象存储分区 |
| manila_disk | vdb3 | Manila 共享存储分区 |
| External_Network | eth1 | 外部网络接口 |
