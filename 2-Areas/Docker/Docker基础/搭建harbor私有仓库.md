...
#解压
tar -zxvf harbor-offline-installer-v2.8.3.tgz -C /opt/
cd /opt/harbor/
ls
common  common.sh    harbor.v2.3.4.tar.gz   install.sh  LICENSE  prepare harbor.yml.tmpl
...
#{{ harbor_install_path }} 替换成harbor仓库对应地址
[Install]
...
Username: admin
Password:
Error response from daemon: Get "https://harbor/v2/": dial tcp 192.168.100.120:443: connect: connection refused
[root@harbor harbor]# docker login http://192.168.100.120

#原因是docker自动使用了https协议访问harbor而我们测试环境使用http协议
#解决方法 添加  "insecure-registries":["HARBOR_IPADDR:80"]
[root@harbor harbor]# cat /etc/docker/daemon.json
{
  "insecure-registries": ["0.0.0.0/0","192.168.100.120:80"],
